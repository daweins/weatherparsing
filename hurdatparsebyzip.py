        ...
        if row[0].startswith("AL"):
            ...
            if(float(row[8]) > 0):
                ...
                for curZip in zipcode:
                    #print(curZip.zipcode)
                    gotMatch = False
                    if curZip.bounds_north is None or curZip.bounds_east is None or curZip.bounds_south is None or curZip.bounds_west is None:
                        #print(f"Skipping zip {curZip.zipcode}")
                        skipMe = True
                    else:
                        curPoly =  Polygon(((curZip.bounds_north,curZip.bounds_east),(curZip.bounds_south,curZip.bounds_east),(curZip.bounds_south,curZip.bounds_west),(curZip.bounds_north,curZip.bounds_west)))
                        # Expanding speeds
                        if poly64.intersects(curPoly):
                            curHur['wind']  = 64
                            gotMatch = True
                        else:
                            if poly50.intersects(curPoly):
                                curHur['wind'] = 50
                                gotMatch = True
                            else:
                                if poly34.intersects(curPoly):
                                    curHur['wind'] = 34
                                    gotMatch = True
                        if gotMatch:
                            curHur['zip'] = curZip.zipcode
                            print(f"{curZip.zipcode} had wind {curHur['wind']}")
